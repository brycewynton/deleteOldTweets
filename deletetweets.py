...
class Deleeeter(object):
    def __init__(self, twitter_api):
        self.twitter_api = twitter_api

    def destroy(self, tweet_id):
        try:
            print("drop tweet %s" % tweet_id)
            self.twitter_api.DropStatus(tweet_id)
            # NOTE: tries to handle rate limits.
            time.sleep(0.5)
...
def delete(tweetjs_path, date, r):
    with io.open(tweetjs_path, mode="r", encoding="utf-8") as tweetjs_file:
        ...
        print("Number of drod tweets: %s\n" % count)


def main():
    parser = argparse.ArgumentParser(description="dr old tweets.")
    ...
    dr(args.file, args.date, args.restrict)
